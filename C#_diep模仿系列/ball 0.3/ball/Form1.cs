...
namespace ball
{
    public partial class Form1 : Form
    {
        ...
        const float MONSTER_CD = 10; // 10/30
        int cd = 0;
        ...
        private void timer1_tick(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < MONSTER_MAX; i++)//monster move
            {
                if (monster[i] != null)
                {
            ...
            for (int i = 0; i < BULLET_MAX; i++)//bullet move
            {
                if (bullet[i] != null)
                {
                    bullet[i].bulletMove(BULLET_SPEED);
                    if (bullet[i].x <= 0 || bullet[i].x >= this.Width ||
                        bullet[i].y <= 0 || bullet[i].y >= this.Width) bullet[i] = null;//borders cheak
                }
            }
            ...
            for (int i = 0; i < BULLET_MAX; i++)//bullet && monster collision cheaks
            {                
                if (bullet[i] == null) continue;
                for (int j = 0; j < MONSTER_MAX; j++)
                {
                    if (monster[j] == null) continue;//if no monster
            ...
            //
            //player.darwObj(backGraphics,player, new Pen(Color.Red, 3));//draw player
            player.darwObj(backGraphics, Color.LightBlue, PLAYER_R);

            int bullet_total = BULLET_MAX, monster_total = 0;

            for (int i = 0; i < BULLET_MAX; i++)//draw bullet
            {
                if (bullet[i] != null)
                {
                    //bullet[i].darwObj(backGraphics, bullet[i], new Pen(Color.Black, 3));
                    bullet[i].darwObj(backGraphics, Color.LightYellow, BULLET_R);
                    bullet_total--;
                }
            }

            for (int i = 0; i < MONSTER_MAX; i++)//draw monster
            {
                if (monster[i] != null)
                {
                    //monster[i].darwObj(backGraphics, monster[i], new Pen(Color.Blue, 3));
                    monster[i].darwObj(backGraphics, Color.OrangeRed, MONSTER_R);
                    monster_total++;
                }
            }
